...
export default function Home() {
  ...
  const initializeGatewayDealAdmin = async () => {
    setIsLoading(true)
    const initializeGatewayDealAdminResponse = await fetch(`${currentDomain}/api/initializeDealAdminGateway`, {
      method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    const initializeDealAdminGatewayData = await initializeGatewayDealAdminResponse.json()
    console.log(initializeDealAdminGatewayData)
    if (initializeDealAdminGatewayData.result == "success") {
      router.push(`/app/dealAdmin/${dealId}`)
    }
    setIsLoading(false)
  }
  ...
  useEffect(() => {
    ...
    if (dealId) {
      (async () => {
        initializeGatewayDealAdmin(dealId)
      })()
    }
  }, [dealId])
      ...
      <div className={styles.loginContainer}>
        <div className={styles.loginComponentsContainer}>
          ...
          <div className={styles.loginButtonContainer}>
            <button type='submit' value="LOGIN" className={styles.sponsorLoginButton} onClick={() => userLogin()}>{userType == "sponsor" ? "Sponsor" : "Investor"} Login</button>
          </div>
